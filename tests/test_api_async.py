...
@pytest.mark.asyncio
async def test_scheduling_info(startup_and_shutdown_uvicorn, base_url, tmp_path):
    """ clear all scheduling. """
    await reset_dispatcher(base_url, str(tmp_path))
...
@pytest.mark.asyncio
async def test_unschedule_active_program(startup_and_shutdown_uvicorn, base_url, tmp_path):
    ...
    await add_action(base_url=base_url, action_name="foo3", action=action3)
    await add_action(base_url=base_url, action_name="foo4", action=action4)
    await add_action(base_url=base_url, action_name="foo5", action=action5)
    await add_action(base_url=base_url, action_name="foo6", action=action6)
    ...
    await add_program(base_url=base_url, program_name="program1", program=program1)
    program2 = PBEProgram().prologue("foo4").epilogue("foo6").body_element("bar", "foo5")
    await add_program(base_url=base_url, program_name="program2", program=program2)
    ...
    await unschedule_active_program(base_url=base_url, program_name= "program1")
...
@pytest.mark.asyncio
async def test_unschedule_active_program_2(startup_and_shutdown_uvicorn, base_url, tmp_path):
    """ test unscheduling the active elements of a program with an UnscheduleActiveProgram action. """
    await reset_dispatcher(base_url, str(tmp_path))
    ...
    await add_action(base_url=base_url, action_name="unscheduleActiveProgram", action=unscheduleActiveProgram)
    ...
    await execute_action(base_url=base_url, action_name="unscheduleActiveProgram")
...
async def unschedule_active_program(base_url: str, program_name: str):
    ...
    response = await get(
        base_url=base_url, path=f"/programs/{program_name}/unschedule_active"
    )
    assert (
        response.status_code == 200
    ), f"failed to unschedule active program ({program_name}) elements"
